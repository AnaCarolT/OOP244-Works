...
namespace sdds {
	...
	std::ostream& Publication::write(std::ostream& os) const
	{
		...
		if (conIO(os)) //if true
		{
			...
			//dot section
			for ( i = 0; i < 30 - int(strlen(m_title)); i++)
			{
				os << ".";
			}
			os << " | "; //end of dots
	...
	std::istream& Publication::read(std::istream& is)
	{
		...
		char answer_title[SDDS_TITLE_WIDTH + 1] = { '\0' }, answer_sheldId[SDDS_SHELF_ID_LEN + 1] = { '\0' };
		...
		if (conIO(is))
		{
			...
			//read the title
			cout << "Title: ";
			is.getline(answer_title, SDDS_TITLE_WIDTH + 1, '\n');
			//date input
			//prompt : "Date: "
			//read the date
			cout << "Date: ";
		...
		else //Otherwise, assume reading begins with libRef attribute as if the type 'P' is not in the file.
		{
			...
			//read the title
			//skip the tab
			is.getline(answer_title, SDDS_TITLE_WIDTH + 1, '\t'); //char needs to be like that or my text goes wrong.
			//read the membership
