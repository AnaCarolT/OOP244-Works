...
// Ana Tirolli     2023-08-06      Publication printing wrongly
// -----------------------------------------------------------
//
...
namespace sdds {
	...
	bool Publication::conIO(std::ios& io) const
	{
		...
		bool ok = false;
		if (&io == &cin || &io == &cout) //comparing adresses
		{
			ok = true; //return true
		}
		return ok;
	...
	std::ostream& Publication::write(std::ostream& os) const
	{
		...
		else //if false
		{
			...
			//shows membershio id, 0 will give N/A
			if (m_membership >= 0) //any number below 5 digits and higher than 0
			{
				os << m_membership;
			}
			else //number is lower than 0
			{
				os << " N/A ";
			}
			os << '\t'; //end of membership id
